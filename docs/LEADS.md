...
- Publicado: receptor `https://bbjgpvuzalevunlugpuk.supabase.co/functions/v1/landing-leads` y migración `20260922210000` confirmados por la tarea backend; Floxia Control ya muestra responsable y notas. Landing en publicación: con aprobación directa del usuario, `FLOXIA_LEADS_WEBHOOK_URL` y `FLOXIA_LEADS_WEBHOOK_SECRET` ya están configuradas como variables sensibles Production del proyecto Vercel `prj_mJIG8zhX8YGdIAM65NsOeztckLGO` (`floxia-ais-projects/floxialanding`, `www.floxia.io`). Pendientes el despliegue y la única prueba real de extremo a extremo. El CTA de WhatsApp continúa publicado.
